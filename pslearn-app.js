...
function _OLD_initDebug(){debugState={idx:0,selected:null,score:0};renderDebugChallenge();}
function _OLD_renderDebugChallenge(){
...
function _OLD_selectDebugLine(n){document.querySelectorAll('.lab-code-line').forEach(l=>l.classList.remove('selected'));document.getElementById(`dline-${n}`).classList.add('selected');debugState.selected=n;document.getElementById('debugCheckBtn').disabled=false;}
function _OLD_checkDebugAnswer(){
...
function _OLD_nextDebugChallenge(){debugState.idx++;renderDebugChallenge();}
...
function _OLD_initEventFlow(){efState.idx=0;loadEFChallenge();}
function _OLD_loadEFChallenge(){const ch=EF_CHALLENGES[efState.idx];document.getElementById('efChallengeName').textContent=ch.name;efState.items=[...ch.events].sort(()=>Math.random()-.5);renderEFList();const res=document.getElementById('efResult');res.classList.remove('show','correct','wrong');res.innerHTML='';document.getElementById('efNextBtn').style.display='none';}
